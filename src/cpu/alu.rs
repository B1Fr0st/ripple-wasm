...
pub fn add(a: u64, b: u64, bits: u32) -> AluResult {
    ...
    let cf = (a & mask).checked_add(b & mask).map_or(true, |v| v & mask != v);
    let sign_bit = 1u64 << (bits - 1);
    ...
    AluResult {
        val: result_masked,
        flags: Flags { cf, zf: result_masked == 0, sf, of, pf: parity(result_masked) },
    }
}
...
pub fn shl(a: u64, count: u64, bits: u32) -> AluResult {
    ...
    if count == 0 {
        let val = a & mask;
        return AluResult { val, flags: flags_from_result(val, bits) };
    }
    ...
    AluResult {
        val,
        flags: Flags { cf, zf: val == 0, sf, of, pf: parity(val) },
    }
}
...
pub fn shr(a: u64, count: u64, bits: u32) -> AluResult {
    ...
    let count = count & 63;
    if count == 0 {
        return AluResult { val: a, flags: flags_from_result(a, bits) };
    }
    let cf = if count <= bits as u64 { ((a >> (count - 1)) & 1) != 0 } else { false };
    let val = a.checked_shr(count as u32).unwrap_or(0);
    let sign_bit = 1u64 << (bits - 1);
    let sf = (val & sign_bit) != 0;
    let of = if count == 1 { (a & sign_bit) != 0 } else { false };
    AluResult {
        val,
        flags: Flags { cf, zf: val == 0, sf, of, pf: parity(val) },
    }
...
pub fn sar(a: u64, count: u64, bits: u32) -> AluResult {
    ...
    let cf = if count <= bits as u64 { ((signed >> (count - 1)) & 1) != 0 } else { signed < 0 };
    let val = (signed >> count.min(63)) as u64 & mask;
    let sf = (val & sign_bit) != 0;
    AluResult {
        val,
        flags: Flags { cf, zf: val == 0, sf, of: false, pf: parity(val) },
    }
}
...
pub fn rol(a: u64, count: u64, bits: u32) -> AluResult {
    ...
    let val = if count == 0 { a } else {
        ((a << count) | (a >> (bits as u64 - count))) & mask
    };
    let cf = val & 1 != 0;
    let sign_bit = 1u64 << (bits - 1);
    let of = if count == 1 { cf != ((val & sign_bit) != 0) } else { false };
    AluResult {
        val,
        flags: Flags { cf, zf: val == 0, sf: (val & sign_bit) != 0, of, pf: parity(val) },
    }
}
...
pub fn ror(a: u64, count: u64, bits: u32) -> AluResult {
    ...
    let of = if count == 1 { (val & sign_bit) != ((val << 1) & sign_bit) } else { false };
    AluResult {
        val,
        flags: Flags { cf, zf: val == 0, sf: (val & sign_bit) != 0, of, pf: parity(val) },
    }
