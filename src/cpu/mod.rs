...
use crate::program::{Program, TEXT_BASE, STACK_TOP, DATA_BASE};
...
impl Cpu {
    pub fn new(prog: Program, debug: bool, max_steps: u64) -> Result<Self, SimError> {
        ...
        Ok(Cpu {
            regs, mem, prog,
            halted: false, steps: 0, debug, max_steps,
            stdout: Vec::new(), stderr: Vec::new(),
            stdin: Vec::new(), stdin_pos: 0,
            exit_code: 0,
        })
    }
    ...
    fn fetch(&self) -> Result<&AsmLine, SimError> {
        ...
        if rip < TEXT_BASE {
            return Err(SimError::Runtime { rip, msg: format!("rip {:#x} below text base", rip) });
        }
        let idx = (rip - TEXT_BASE) as usize;
        self.prog.instructions.get(idx).ok_or_else(|| SimError::Runtime {
            rip,
    ...
    pub fn step(&mut self) -> Result<bool, SimError> {
        ...
        let halt = execute::execute(&instr, &mut self.regs, &mut self.mem, &self.prog.symbols, &mut io)?;
        self.steps += 1;
